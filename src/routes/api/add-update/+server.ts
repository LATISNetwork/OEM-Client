...
export const POST = (async ({ fetch, request, cookies }) => {
	...
	const deviceData = (await admin.firestore().collection(userData?.oemID).doc(body.deviceID).get()).data();
	console.log('deviceData', deviceData);
	if (!deviceData){
		await admin.firestore().collection(userData?.oemID).doc(body.deviceID).set({updates: [body]});
		return new Response('Ok');
	}
	const oemID = userData?.oemID;
	console.log('oemID', oemID);
	try {
		let res1 = await admin.firestore().collection(oemID).doc(body.deviceID).set({updates: [...deviceData?.updates, body]});
